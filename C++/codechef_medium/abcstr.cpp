...
int main()
{
    ...
    cin>>s;
    int n = s.length();
    // int a[n];//will store number of A's till i
    // int b[n];//will store #of B-#of A till i
    int c[n];//will store #of C-#of A till i
    map<pair<int,int>,int> mp;
    int na=0,nb=0,nc=0;
    if(s[0]=='A') na++; else if (s[0]=='B')nb++;else nc++;
    c[0]=nc-na;
    long long ans=0;
    mp[make_pair(0,0)]++;
    for(int i=0;i<n;i++)
    {
        ...
        {
            na++;
        } else if (s[i]=='B'){nb++;}else {nc++;}
        ans+=mp[make_pair(nb-na,nc-na)];
        mp[make_pair(nb-na,nc-na)]++;
    }
    // for(int i=0;i<n;i++)
    // {
    //     cout<<c[i]<<" ";
